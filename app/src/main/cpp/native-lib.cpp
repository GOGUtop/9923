...
#include "node.h"
#define LOG_TAG "GouGuNode"
#define LOGI(...) __android_log_print(ANDROID_LOG_INFO, LOG_TAG, __VA_ARGS__)
extern "C" JNIEXPORT jint JNICALL
Java_com_xixijiuguan_gougulocaltavern_NodeMobileNative_startNodeWithArguments(JNIEnv *env, jclass, jobjectArray arguments) {
    int argc = env->GetArrayLength(arguments);
    std::vector<std::string> args;
    std::vector<char*> argv;
    args.reserve(argc); argv.reserve(argc);
    ...
    LOGI("Starting embedded Node argc=%d", argc);
